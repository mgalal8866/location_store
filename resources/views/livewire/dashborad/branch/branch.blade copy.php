        ...
        <div wire:ignore class="card-header p-0 border-bottom-0">
                ...
                <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
                    @foreach($stores->branch as $branch)
                        <li class="nav-item">
                                <a class="nav-link {{ $loop->index == 0 ? 'active' :'' }}" id="b{{$branch->id . $branch->slug }}-tab"
                                    data-toggle="pill" href="#b{{$branch->id . $branch->slug }}"role="tab"
                                    aria-controls="b{{$branch->id .  $branch->slug }}" aria-selected="true" >Branch {{$loop->index + 1}}
                               </a>
                        </li>
                    @endforeach
                </ul>
            ...
            <div class="tab-content" id="custom-tabs-four-tabContent">
                @forelse($stores->branch as $branch)
                        ...
                        <div  wire:ignore.self class="tab-pane fade{{ $loop->index == 0 ? ' show active' :'' }}"
                            id="b{{$branch->id . $branch->slug }}"
                            role="tabpanel"
                            aria-labelledby="b{{$branch->id . $branch->slug }}-tab"
                            >
